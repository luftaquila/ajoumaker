...
서버 IP를 브라우저 주소창에 입력하거나, 인증용 태블릿PC의 바탕화면 바로가기를 이용해 접속합니다.
...
## 4. 사용 인증 서버 관리
### 인증 데이터는 서버 컴퓨터의 로컬 하드 디스크 외 어디에도 백업되지 않습니다.  
반드시 주기적으로 백업하십시오.
...
1. 컴퓨터를 재부팅하고, `Win + R` - `cmd`를 입력해 명령 프롬프트를 실행합니다.
    콘솔 명령 프롬프트에 `node`를 입력하고 엔터를 누르십시오.
...
* 배치 파일을 이용해 서버 실행을 자동화할 수 있습니다.  
ex) `server.js` 파일을 `D:\메이커스페이스 서버` 경로에 저장한 경우
```Batchfile
D:
cd "메이커스페이스 서버"
cls
node server
```
* 위 내용을 메모장에 붙여넣고 `다른 이름으로 저장` - `<파일명>.bat`으로 저장하여 배치 파일을 생성합니다.
* 배치 파일을 시작 프로그램에 등록하면 부팅 시 자동으로 실행됩니다.  
* 배치 파일을 바탕 화면 바로가기로 만들어 두면 실수로 닫았을 때 편리하게 재실행할 수 있습니다.
